...
class ClientAWS(Client):
    ...
    def get_ssh_proxy_command(self):
        ...
        if self.config.aws.teacher_access == "endpoint":
            ...
            access_ip = self._get_bastion_host_ip()
            username = self._get_bastion_host_username()
    ...
    def _get_bastion_host_ip(self):
        ...
        """
        if self.config.aws.teacher_access == "host":
            return self.get_machine_public_ip(self._get_bastion_host_name())
        return None
    ...
    def _get_bastion_host_name(self):
        ...
        """
        if self.config.aws.teacher_access == "host":
            return f"{self.description.institution}-{self.description.lab_name}-bastion_host"
        return None
