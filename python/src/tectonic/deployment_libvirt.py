...
class LibvirtDeployment(Deployment):
    terraform_module_path = os.path.join(Deployment.base_dir, "terraform", "modules", "gsi-lab-libvirt")
    terraform_services_module_path = os.path.join(Deployment.base_dir, "services", "terraform", "services-libvirt")
    packetbeat_module_path = os.path.join(Deployment.base_dir, "elastic", "monitoring", "traffic", "libvirt")
    ...
    def destroy_infraestructure(self, instances):
        ...
        if instances is not None:
            ...
            if len(self.description.get_services_to_deploy()) > 0:
                ...
                self._destroy_services(self.terraform_services_module_path, self.get_deploy_services_vars(), None)
                if self.description.deploy_elastic and self.description.monitor_type == "traffic":
