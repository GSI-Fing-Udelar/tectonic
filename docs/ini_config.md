...
### [moodle] section:
* `version`: Moodle version to use. Use `latest` for latest version or assign a specific version. Default: `4.5.8`. All tests were performed on version `4.5.8` so we recommend using it. However, it may also work for older or newer Moodle versions.
