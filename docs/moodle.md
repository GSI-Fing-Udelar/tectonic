# Moodle
In order to upload resources to Moodle you must provide a `moodle` directory with Moodle backups files (mbz).
...
Once the course is created, content must be added. There are different types of content, such as quizzes, presentations, documents, and more. To add content, follow these steps:

1. Log in as an administrator or teacher and go into your course.
2. Toggle Edit mode top right.
3. To add files such as documents or presentations, drag and drop from your desktop.
...
Once the courses have been created, a backup of each one must be made. Follow these steps:
...
Do not include users when taking the backup.
